...
function makeWorker(bodyFunction) {
  ...
  return function runTask(params, cb = () => {}) {
    const entityId = params.projectId;
...
function xlsxWorker() {
  ...
  function addResultLabels(sheet, labels, investorsCount) {
    labels.forEach((label, idx) =>
      sheet
        .row(idx + 3 + investorsCount)
        .cell(1)
        .value(label)
    );
  }
  ...
  const roundValues = (data, roundId, cols, cursor) =>
    data.investors.forEach(({ id }, row) => {
  ...
  const totalValues = (data, roundId, cols, cursor, investorsCount) =>
    cols.forEach(([colType, , format], col) => {
      cursor
  ...
  const addSplitParams = (data, roundId, cursor, investorsCount) =>
    cursor
  ...
  const addJkissParams = (data, roundId, cursor, investorsCount) =>
    cursor
      .relativeCell(9 + investorsCount, 0)
  ...
  async function build(data) {
    ...
    const investorsCount = data.investors.length;
    const sheet = wb.sheet(0);

    makePanes(sheet, data.meta.title);
    addInvestors(sheet, data.investors);
    addResultLabels(sheet, data.resultLabels, investorsCount);
    ...
    for (const [roundId, { type, name, date }] of data.meta.rounds) {
      ...
      totalValues(data, roundId, cols, cursor, investorsCount);
      addRoundResults(
        cursor.relativeCell(3 + investorsCount, 0),
        data.data[roundId].roundResults,
        colSpan
      );

      if (type === "split") {
        addSplitParams(data, roundId, cursor, investorsCount);
      }
      if (type === "j-kiss") {
        addJkissParams(data, roundId, cursor, investorsCount);
      }
  ...
  self.addEventListener("message", async (event) => {
    let { taskId, data, entityId } = event.data;
...
export default function exportExcel(id) {
  ...
  const data = {
    ...
    paramsLabels: ["ﾊﾞﾘｭｴｰｼｮﾝｷｬｯﾌﾟ", "割引率", "分割数"].map(trans),
    resultLabels: [
      "合計",
      "株価",
      "調達金額",
  ...
  conclude(cps(requestGenerate, { data, projectId: id }), (err, buf) => {
    // TODO: handle error
